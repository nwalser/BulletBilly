...
is_defect = np.where(np.abs(wall_offset) > 0.005, True, False)
...
## OFFSET MAP
ax3 = fig.add_subplot(gs[1:7, 3:])
ax3.set_title("Section Data")
X, Y = np.meshgrid(edges, np.arange(wall_offset.shape[1] + 1))
hm = ax3.pcolormesh(X, Y, wall_offset.T, cmap="viridis", vmin=0, vmax=0.005, shading='flat', label="Hello")
...
def onselect(xmin, xmax):
    mask = (entry_depth >= xmin) & (entry_depth <= xmax)
    X_slice = wall_distance_m[mask] * np.cos(angles_grid[mask])
    Y_slice = wall_distance_m[mask] * np.sin(angles_grid[mask])
    Z_slice = depth_grid[mask]
    color_slice = cm.viridis(norm(wall_offset[mask]))

    ax2.clear()
    ax2.plot_surface(Y_slice, Z_slice, X_slice, facecolors=color_slice, rstride=4, cstride=4, shade=True, axlim_clip=True)
    ax2.view_init(elev=15, azim=-60)
    ax2.zaxis.set_major_formatter(FuncFormatter(lambda val, _: f"{val*1000:.0f} mm"))
    ax2.yaxis.set_major_formatter(FuncFormatter(lambda val, _: f"{val*1000:.0f} mm"))
    ax2.xaxis.set_major_formatter(FuncFormatter(lambda val, _: f"{val*1000:.0f} mm"))
